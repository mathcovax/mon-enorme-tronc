...
import { POST, DELETE } from "./{productSheetId}";
import { duploTesting } from "@test/setup";

describe("POST /category/{categoryId}/product-sheet/{productSheetId}", () => {
	beforeEach(() => {
		MockPrisma.resest();
	});

	it("attribute category to product", async () => {
		const spy = vi.fn(() => null);
		MockPrisma.set("product_sheet_to_category", "create", spy);

		const res = await duploTesting
			.testRoute(POST("POST", "/category/1234/product-sheet/1234"))
	...
	it("attribute category to product - category notfound", async () => {
		const res = await duploTesting
			.testRoute(POST("POST", "/category/1234/product-sheet/1234"))
			.setRequestProperties({
				params: {
	...
	it("attribute category to product - product_sheet notfound", async () => {
		const res = await duploTesting
			.testRoute(POST("POST", "/category/1234/product-sheet/1234"))
			.setRequestProperties({
				params: {
